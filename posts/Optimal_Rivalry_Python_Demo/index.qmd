...
Edit the Python code below and click **Run Code** to explore the model. First run
**Load the matchup data**, then try any of the three experiments. The first visit
may take a little while to load Python and its packages; an internet connection
is needed to load the data. Your edits affect your browser session only.

Use **Start Over** on a code block to restore its original code. To reset all
variables as well, reload the page and run the data-loading block again.

### Load the matchup data

This loads the same source matrices used above and recreates the payoff matrix.
The list of hero names in the output is available for copying into your teams.
...
Change the six names in `opponent` and run the code. You can also exclude heroes
from **your own** selection with `unavailable`. With no role or synergy constraints,
selecting the six highest individual contributions gives the exact best response
to the fixed opponent team.
